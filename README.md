...
2.3.0 新增 PDF 阅读及调色，改进高 DPI PNG 导出、缩放时的细线显示和自动选字；常见图表的颜色替换导出可保留矢量文字和路径。

EPS Live Viewer 是面向科研绘图快速迭代的轻量级跨平台桌面查看器。它能实时查看 EPS/PS/PDF，也能直接打开 PNG/JPG，在同一个界面中完成相邻文件比较、多页浏览、旋转、反色、替换颜色、图片导出和视频制作。

## 主要功能

- EPS/PS/PDF 矢量预览：放大时按当前视图重新渲染；瓦片留边与更高采样率改善细坐标轴在不同缩放下的显示。PDF 阅读无需 Ghostscript。
...
发布包已包含阅读与调色所需组件。当前不支持输入密码解锁受保护的 PDF，也不提供 OCR、修改文字或移动图形对象。PDF 文件较复杂时，个别效果会采用兼容位图导出。
