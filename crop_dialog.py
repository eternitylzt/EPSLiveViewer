...
class CropSelectionDialog(QDialog):
    ...
    def __init__(
        ...
        super().__init__(parent)
        self.setWindowTitle("选择视频区域")
        self.resize(900, 680)
        layout = QVBoxLayout(self)
        intro = QLabel(
            f"参考帧：{filename}　{image.width()} × {image.height()} 像素\n"
        ...
        full_button = QPushButton("使用完整图像")
        ...
        buttons.button(QDialogButtonBox.StandardButton.Ok).setText("确认区域")
    ...
    def _update_selection_label(self) -> None:
        ...
        if crop is None:
            self._selection_label.setText(f"完整图像：{width} × {height} 像素")
        else:
            self._selection_label.setText(
                f"选定区域：{width} × {height} 像素　"
                f"（宽 {crop[2] * 100:.1f}%，高 {crop[3] * 100:.1f}%）"
            )
